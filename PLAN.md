...
3. **Build Cart Service vertical slice**
   ...
   - security checks `SEC-SCOPE-WITHOUT-RELATIONSHIP`,
     ...
     `SEC-SECURITY-TRACE-EVIDENCE`, and `SEC-RELATIONSHIP-REMOVAL-IMMEDIATE`.
...
| Cart vertical slice | Cart UI and cart-service run through gateway; JWT, scope, and SpiceDB gates are traced; `SEC-NO-BROWSER-TOKENS`, `SEC-NON-COMMERCE-AUD`, `SEC-SCOPE-WITHOUT-RELATIONSHIP`, `SEC-RELATIONSHIP-WITHOUT-SCOPE`, `SEC-SPOOFED-IDENTITY-HEADERS`, `SEC-BROWSER-AUTHORIZATION-OVERWRITTEN`, `SEC-SECURITY-TRACE-EVIDENCE`, and `SEC-RELATIONSHIP-REMOVAL-IMMEDIATE` pass live; architecture checks for controller/service/domain/persistence/security boundaries pass; no-browser-token guard remains green. |
...
| Order + payment services | Checkout, order read, support read, owner cancel, and payment S2S flow pass; payment is internal-only and not exposed through browser APISIX routes; Payment Service accepts only Order Service client-credentials tokens with `aud=payment-service`, `azp/client_id=order-service`, and `payments:authorize`; user/browser `commerce-api` tokens are rejected by payment; checkout claims the idempotency key/body before calling payment; same-key/same-body replay returns the same order without double-authorizing payment; same-key/different-body collision rejects before payment is called; `SEC-PAYMENT-NO-BROWSER-ROUTE`, `SEC-PAYMENT-WRONG-CLIENT`, `SEC-PAYMENT-REJECTS-USER-TOKEN`, `SEC-CHECKOUT-IDEMPOTENT-REPLAY`, and `SEC-CHECKOUT-IDEMPOTENCY-COLLISION` pass live; architecture checks remain green. |
