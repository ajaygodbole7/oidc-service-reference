...
info "order-service verifier"
info "evidence is bounded: Maven goal and test result only; no tokens or secrets are printed"

[ -f pom.xml ] || die "root pom.xml missing; order-service must build with commerce-security-common in the same reactor"
[ -f order-service/pom.xml ] || die "order-service/pom.xml missing"
...
success "HARNESS-ORDER-SERVICE passed"
