...
info "payment-service verifier"
info "evidence is bounded: Maven goal and test result only; no tokens or secrets are printed"

[ -f pom.xml ] || die "root pom.xml missing; payment-service must build with commerce-security-common in the same reactor"
[ -f payment-service/pom.xml ] || die "payment-service/pom.xml missing"
...
auth-service/mvnw -B -f pom.xml -pl payment-service -am clean test

success "HARNESS-PAYMENT-SERVICE passed"
