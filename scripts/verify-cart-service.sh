...
info "cart-service verifier"
info "evidence is bounded: Maven goal and test result only; no tokens or secrets are printed"

[ -f pom.xml ] || die "root pom.xml missing; cart-service must build with commerce-security-common in the same reactor"
[ -f cart-service/pom.xml ] || die "cart-service/pom.xml missing"
...
auth-service/mvnw -B -f pom.xml -pl cart-service -am clean test

if [ "${CART_SPICEDB_LIVE:-}" = "1" ] || [ "${CART_SPICEDB_LIVE:-}" = "true" ]; then
  sh scripts/verify-cart-spicedb-live.sh
fi

success "HARNESS-CART-SERVICE passed"
