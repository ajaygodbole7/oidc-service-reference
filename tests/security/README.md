...
| `SEC-SECURITY-TRACE-EVIDENCE` | Prove harness-only Security Trace evidence shows the four cart gates. | `sh tests/security/verify-cart-security-live.sh SEC-SECURITY-TRACE-EVIDENCE` | Read-only local evidence includes trace ID, gateway session/JWT proof, scope trace, and SpiceDB resource trace without token material. | Propagate `X-Trace-Id` through APISIX and expose bounded cart evidence only under `test-fixture`. |
