...
class CommercePrincipalFilterTest {
  ...
  void missingBearerToken_returnsEnrichedProblem401() throws Exception {
    ...
    assertThat(body.get("type").asString()).isEqualTo("https://errors.example.com/cart/invalid-token");
    ...
    assertThat(body.get("traceId").asString()).isEqualTo("trace-cart-1");
  }

  @Test
  void invalidBearerToken_returnsEnrichedProblem401() throws Exception {
    MDC.put("traceId", "trace-cart-2");
    ...
    assertThat(body.get("errorCode").asString()).isEqualTo("INVALID_TOKEN");
    assertThat(body.get("traceId").asString()).isEqualTo("trace-cart-2");
  }
