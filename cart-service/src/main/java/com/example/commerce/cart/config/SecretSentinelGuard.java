 ...
 * cart-service seeds dev defaults for its datasource password and SpiceDB preshared key so the
 * stack is zero-config locally; each default carries the marker {@value SecretSentinel#MARKER}
 * (shared via {@link SecretSentinel}).
 *
...
class SecretSentinelGuard {
  ...
  void validateOnStartup() {
    ...
    String spicedbPresharedKey = env.getProperty("cart.spicedb.preshared-key");
    ...
    if (!SecretSentinel.isLocalProfile(env.getActiveProfiles())) {
      ...
      throw new IllegalStateException(
          "Refusing to run with default dev secrets outside a local profile. "
              + "Set CART_DATASOURCE_PASSWORD and CART_SPICEDB_PRESHARED_KEY explicitly.");
    }

    if (datasourceIsSentinel) {
      log.warn("CART_DATASOURCE_PASSWORD is the local-dev sentinel — replace before any non-local deploy.");
    }
    if (spicedbIsSentinel) {
      log.warn("CART_SPICEDB_PRESHARED_KEY is the local-dev sentinel — replace before any non-local deploy.");
    }
  }
}
